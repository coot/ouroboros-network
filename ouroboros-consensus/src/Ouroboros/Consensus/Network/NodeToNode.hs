...
mkHandlers
    ...
    Handlers {
      ...
      , hPeerSharingClient = \_version _peer -> peerSharingClient
...
mkApps kernel Tracers {..} mkCodecs ByteLimits {..} genChainSyncTimeout ReportPeerMetrics {..} Handlers {..} =
  ...
  where
    ...
    aPeerSharingClient version _controlMessageSTM them channel = do
      labelThisThread "PeerSharingClient"
      bracketPeerSharingClient (getPeerSharingRegistry kernel) them
        $ \controller -> do
          psClient <- hPeerSharingClient version them controller
          ((), trailing) <- runPeerWithLimits
            nullTracer
