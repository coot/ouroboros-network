...
data Interfaces ntnFd ntnAddr ntnVersion ntnVersionData
    ...
    Interfaces {
        ...
        diInstallSigUSR1Handler
          :: forall mode x y.
             NodeToNodeConnectionManager mode ntnFd ntnAddr ntnVersion ntnVersionData m x y
          -> m (),
                ...
                let connectionManagerArguments
                    ...
                    connectionManagerArguments =
                      ConnectionManagerArguments {
                          ...
                          connectionDataFlow    = \n h -> diNtnDataFlow n (hVersionData h),
                ...
                withConnectionManager
                  ...
                  $ \(connectionManager
                      :: NodeToNodeConnectionManager
                           InitiatorMode ntnFd ntnAddr ntnVersion ntnVersionData m a Void)
                    -> do
              ...
              HasInitiatorResponder
                ...
                let connectionManagerArguments
                    ...
                    computePeerSharingPeers :: STM m (PublicPeerSelectionState ntnAddr)
                                            -> PeerSharingAmount
                                            -> m [ntnAddr]
                    computePeerSharingPeers readPublicState amount = do
                      publicState <- atomically readPublicState
                      let availableList = toList (availableToShare publicState)
                      return (take (fromIntegral amount) availableList)
                ...
                withConnectionManager
                  ...
                  $ \(connectionManager
                        :: NodeToNodeConnectionManager
                             InitiatorResponderMode ntnFd ntnAddr ntnVersion ntnVersionData m a ()
                     ) -> do
                  diInstallSigUSR1Handler connectionManager
                 ...
                 diInstallSigUSR1Handler
                   :: forall mode x y.
                      NodeToNodeConnectionManager mode Socket RemoteAddress
                                                  NodeToNodeVersion NodeToNodeVersionData IO x y
                   -> IO ()
#ifdef POSIX
